...
This option is kept for historical reasons whe default behavior was
to download packages when they are needed, not all at once.
You can also use this option to specify a directory where the files should
be downloaded (default is /var/cache/urpmi which could be too small to hold all the files).

=item B<--no-download-all>

By default, urpmi will first download all the needed packages and proceed
to install them if it managed to download them all. With this option,
urpmi will download packages when they are needed.
...
=item B<--force-req-update>
Try to update all package requirements - that is, even if some requirements
are already installed, look for their updates. This is the default behavior.

This option is useful if you have 'no-force-req-update' set in
urpmi.cfg, but want to enable update of package requires in particular
run.

=item B<--no-force-req-update>
Do not update package requirements if they are already installed
