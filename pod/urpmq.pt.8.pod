...
=head1 SYNOPSIS
    ...
    urpmq [options] [nomes_dos_pacotes | ficheiros_rpm...]

=head1 DESCRIPTION

O urpmq é uma ferramenta para aceder e consultar a base de dados urpmi.
Podeser usada para listar os pacotes disponíveis nas várias médias urpmi, ou
...
Nenhum ficheiro rpm será encontrada noutras médias.
...
agrupamento. Desta maneira, a média C<media1> será considerada em primeiro
lugar,depois a média C<media2>, e por ai adiante. As médias que não estão
...
(procurar através de pacotes virtuais).
...
Mostra também a versão e lançamento de cada pacote listado.

=item B<-f>

Mostra também a versão, lançamento e arquitectura de cada pacote listado.
